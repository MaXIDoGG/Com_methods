...
namespace Com_Methods
{
    ...
    public class Matrix : IMatrix
    {
        ...
        //нахождение подматрицы без указанных строки и столбца
        ...
        public double CalculateDeterminantRecursive()
        {
            ...
            for (int i = 0; i < M; i++)
            {
                ...
                determinant += Math.Pow(-1, i) * Elem[0][i] * subMatrix.CalculateDeterminantRecursive();
            }
        ...
        {
            Matrix matrixAdditions = new Matrix(M, N);
            for (int i = 0; i < M; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    matrixAdditions.Elem[i][j] =
                        Math.Pow(-1, i + j) * CreateSubMatrix(i, j).CalculateDeterminantRecursive();
                }
            }

            return matrixAdditions.Transpose_Matrix() * (1/CalculateDeterminantRecursive());
        }
        
        //нахождение обусловленности
        public double Conditionality()
        {
            return Normi() * InverseMatrix().Normi();
        }
        
        //матричная норма (бесконечная норма)
        public double Normi()
        {
            double maxSum = 0;
            double sum = 0;
